...
case $1 in
   f|fullcone)
      ...
      iptables --table nat --append POSTROUTING --out-interface eth1 --source 192.168.0.2 --jump SNAT --to-source 1.2.66.2
      iptables --table nat --append POSTROUTING --out-interface eth1 --source 192.168.0.3 --jump SNAT --to-source 1.2.66.3

      iptables --table nat --append PREROUTING --in-interface eth1 --destination 1.2.66.2 --jump DNAT --to-destination 192.168.0.2
      iptables --table nat --append PREROUTING --in-interface eth1 --destination 1.2.66.3 --jump DNAT --to-destination 192.168.0.3
      ;;
